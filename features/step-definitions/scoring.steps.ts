...
Given(
  'a puzzle with letters {string} and center {string}',
  function (this: SanakennoWorld, lettersStr: string, center: string) {
...
When(
  ...
  function (this: SanakennoWorld, word: string) {
    ...
    const isPangram = [...this.allLetters].every((c: string) =>
      word.includes(c),
    );
...
When(
  'the player submits {string} again',
  function (this: SanakennoWorld, word: string) {
    if (this.foundWords.includes(word)) {
      this.message = 'Löysit jo tämän!';
      this.lastScoreIncrease = 0;
...
Then(
  'the score should increase by {int}',
  function (this: SanakennoWorld, expectedIncrease: number) {
...
Then(
  'the total score should be {int}',
  function (this: SanakennoWorld, expectedTotal: number) {
...
Then(
  ...
  function (this: SanakennoWorld, expectedMessage: string) {
