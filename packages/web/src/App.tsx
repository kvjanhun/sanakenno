...
function App() {
  ...
  const handleCopyPwaCode = useCallback(async (token: string) => {
    await share.copyToClipboard(token);
    setShowPwaInstructions(true);
  }, []);

  const handleCloseConnectBanner = useCallback(() => {
  ...
  useScrollBoundaryGuard(mainScrollRef);
  ...
  return (
    ...
    >
      ...
      <main
        ref={mainScrollRef}
        data-game-scroll
        className="absolute left-0 right-0 bottom-0 z-10 overflow-y-auto overscroll-contain game-scroll-surface"
        style={{
